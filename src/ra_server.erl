...
-type ra_server_state() ::
        #{
          ...
          aux_state => term(),
          condition => #{
                         predicate_fun := ra_await_condition_fun(),
...
init(#{
    ...
    SystemConfig = maps:get(system_config,
                            Config,
    ...
    MaxPipelineCount = maps:get(max_pipeline_count,
                                Config,
                                DefaultMaxPipelineCount),
    DefaultMaxAERBatchSize = maps:get(default_max_append_entries_rpc_batch_size,
                                      SystemConfig,
                                      ?AER_CHUNK_SIZE),
    MaxAERBatchSize = maps:get(max_append_entries_rpc_batch_size,
                               Config,
    ...
    NonVoter = get_membership(Cluster0,
                              Id,
                              UId,
...
recover(#{
    ...
    {#{
       log := Log0,
       cfg := #cfg{effective_machine_version = EffMacVerAfter}
      } = State1,
     _} =
    ...
    {State, Log1} = ra_log:fold(FromScan,
                                ToScan,
...
handle_leader(pipeline_rpcs, State0) ->
    ...
    {leader, State, Effects};
handle_leader(#install_snapshot_rpc{
                term = Term,
                leader_id = Leader
               } = Evt,
              #{
...
handle_leader(#append_entries_rpc{term = Term} = Msg,
  ...
  when Term > CurTerm ->
    ?INFO("~ts: leader saw append_entries_rpc from ~w for term ~b "
          "abdicates term: ~b!",
          [LogId,
           Msg#append_entries_rpc.leader_id,
           Term,
           CurTerm]),
    {follower, update_term(Term, State0#{leader_id => undefined}),
               [{next_event, Msg}]};
handle_leader(#append_entries_rpc{term = Term},
              #{
                current_term := Term,
                cfg := #cfg{log_id = LogId}
               }) ->
  ...
  when Term >= CurTerm ->
    ...
    case has_log_entry_or_snapshot(PLIdx, PLTerm, Log00) of
        ...
        {term_mismatch, OtherTerm, Log0} ->
            ...
            {Reply, State} = mismatch_append_entries_reply(Term,
                                                           LastApplied,
                                                           State0),
            Effects = [cast_reply(Id, LeaderId, Reply) | Effects0],
            {await_condition,
...
handle_follower(#heartbeat_rpc{
                  query_index = RpcQueryIndex,
                  term = Term,
                  leader_id = LeaderId
                 },
                #{
...
handle_follower({_PeerId, #heartbeat_reply{term = TheirTerm}},
    ...
    Term = max(TheirTerm, CurTerm),
    {follower, update_term(Term, State), []};
handle_follower(#install_snapshot_rpc{
                  term = Term,
                  meta = #{
                           index := LastIndex,
                           term := LastTerm
...
handle_follower(#install_snapshot_rpc{
  ...
  when Term >= CurTerm andalso
    ...
    {receive_snapshot, update_term(Term,
                                   State0#{
                                     log => Log,
                                     leader_id => LeaderId
                                    }),
...
handle_receive_snapshot(#install_snapshot_rpc{
                          term = Term,
                          meta = #{
                                   index := SnapIndex,
                                   machine_version := SnapMacVer,
  ...
  when Term >= CurTerm ->
    ...
    SnapState0 = ra_log:snapshot_state(Log0),
    {ok, SnapState} = ra_snapshot:accept_chunk(Data,
                                               Num,
                                               ChunkFlag,
                                               SnapState0),
    Reply = #install_snapshot_result{
        ...
        last ->
            ...
            Cfg = case SnapMacVer > CurEffMacVer of
                      true ->
                          ...
                          Cfg0#cfg{
                            effective_machine_version = SnapMacVer,
                            machine_versions = [{SnapIndex, SnapMacVer} | MachineVersions],
...
overview(#{
           cfg := #cfg{effective_machine_module = MacMod} = Cfg,
           log := Log,
           machine_state := MacState,
           aux_state := Aux,
...
handle_aux(RaftState,
    ...
    %% NEW API
    case ra_machine:handle_aux(MacMod,
                               RaftState,
                               Type,
                               Cmd,
                               Aux0,
...
evaluate_commit_index_follower(#{
                                 commit_index := CommitIndex,
                                 cfg := #cfg{id = Id},
                                 leader_id := LeaderId,
    ...
    case catch apply_to(ApplyTo, State0, Effects0) of
        {delete_and_terminate, State1, Effects} ->
            ...
            {delete_and_terminate, State1,
                                   [cast_reply(Id, LeaderId, Reply) | filter_follower_effects(Effects)]};
...
make_pipelined_rpc_effects(#{
    ...
    maps:fold(
              ...
              case NumInFlight < MaxPipelineCount of
                  true ->
                      ...
                      EntryCache = case Effs of
                                       [{send_rpc, _,
                                                   #append_entries_rpc{entries = Es}} | _] ->
                                           Es;
                                       _ ->
                                           []
                                   end,
                      ...
                      {NewNextIdx, Eff, S} =
                          make_rpc_effect(PeerId,
                                          Peer0,
                                          BatchSize,
                                          S0,
                                          EntryCache),
                      Peer = Peer0#{
                               next_index => NewNextIdx,
      ...
      {State0, false, add_flush_event(State0, Effects0)},
      Cluster).
...
make_rpc_effect(PeerId,
    ...
    case ra_log:fetch_term(PrevIdx, Log0) of
        {PrevTerm, Log} when is_integer(PrevTerm) ->
            make_append_entries_rpc(PeerId,
                                    PrevIdx,
                                    PrevTerm,
                                    MaxBatchSize,
                                    State#{log => Log},
...
checkpoint(Index,
           ...
           State = #{log := Log0, cluster := Cluster}) ->
    MacVersion = index_machine_version(Index, State),
    {Log, Effects} = ra_log:checkpoint(Index,
                                       Cluster,
                                       MacVersion,
                                       MacState,
                                       Log0),
...
persist_last_applied(#{
                       persisted_last_applied := PLA,
                       last_applied := LA
                      } = State) when LA =< PLA ->
...
handle_down(leader, machine, Pid, Info, State)
  when is_pid(Pid) ->
    ...
    {leader, State, [Eff]};
handle_down(RaftState,
            snapshot_sender,
            Pid,
            Info,
            #{cfg := #cfg{log_id = LogId}} = State)
  when (RaftState == leader orelse
        RaftState == await_condition) andalso
       is_pid(Pid) ->
...
-spec handle_node_status(ra_state(),
                         machine | aux,
                         node(),
                         nodeup | nodedown,
                         term(),
...
update_term_and_voted_for(Term,
                          VotedFor,
                          #{
                            cfg := #cfg{uid = UId} = Cfg,
                            current_term := CurTerm
...
state_query(voters, #{cluster := Cluster}) ->
    maps:fold(fun(K, V, Acc) ->
                      case maps:get(voter_status, V, undefined) of
                          undefined -> [K | Acc];
                          S ->
                              case maps:get(membership, S, undefined) of
                                  undefined -> [K | Acc];
                                  voter -> [K | Acc];
                                  _ -> Acc
                              end
...
apply_with({Idx, Term, {'$usr', CmdMeta, Cmd, ReplyMode}},
    ...
    case ra_machine:apply(Module, Meta, Cmd, MacSt) of
        {NextMacSt, Reply, AppEffs} ->
            {Effects, Notifys} = add_reply(CmdMeta,
                                           Reply,
                                           ReplyMode,
    ...
    case NextMacVer > OldMacVer of
        true when IsOk ->
            ...
            apply_with({Idx,
                        Term,
                        {'$usr', Meta,
                                 {machine_version, OldMacVer, NextMacVer},
                                 none}},
...
reset_query_index(#{cluster := Cluster} = State) ->
    State#{
      cluster => maps:map(fun(_PeerId, Peer) ->
                                  Peer#{query_index => 0}
                          end,
                          Cluster)
     }.
...
heartbeat_rpc_effects(Peers, Id, Term, QueryIndex) ->
    lists:filtermap(fun({PeerId, Peer}) ->
                            heartbeat_rpc_effect_for_peer(PeerId,
                                                          Peer,
                                                          Id,
                                                          Term,
                                                          QueryIndex)
...
heartbeat_rpc_quorum(NewQueryIndex,
                     PeerId,
...
process_pending_consistent_queries(#{pending_consistent_queries := []} = State0,
                                   Effects0) ->
    {State0, Effects0};
process_pending_consistent_queries(#{
                                     cluster_change_permitted := true,
                                     pending_consistent_queries := Pending
                                    } = State0,
                                   Effects0) ->
