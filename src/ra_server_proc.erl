...
-type ra_command() :: {ra_server:command_type(),
                       term(),
...
do_init(#{
    ...
    #{names := Names} = SysConf,
    ok = ra_directory:register_name(Names,
                                    UId,
                                    self(),
                                    maps:get(parent, Config, undefined),
                                    Key,
                                    ClusterName),
    ...
    FlushCommandsSize = maps:get(low_priority_commands_flush_size,
                                 SysConf,
                                 ?FLUSH_COMMANDS_SIZE),
    SnapshotChunkSize = maps:get(snapshot_chunk_size,
                                 SysConf,
                                 ?DEFAULT_SNAPSHOT_CHUNK_SIZE),
    ReceiveSnapshotTimeout = maps:get(receive_snapshot_timeout,
                                      SysConf,
...
leader(EventType, {leader_cast, Msg}, State) ->
    leader(EventType, Msg, State);
leader(EventType,
       {command, normal, {CmdType, Data, ReplyMode}},
...
leader(EventType,
       {command, low, {CmdType, Data, ReplyMode}},
       #state{
         conf = Conf,
         low_priority_commands = Delayed
        } = State0) ->
...
leader(EventType, {aux_command, Cmd}, State0) ->
    {_, ServerState, Effects} = ra_server:handle_aux(?FUNCTION_NAME,
                                                     EventType,
                                                     Cmd,
...
leader(EventType,
       flush_commands,
       #state{
         conf = #conf{flush_commands_size = Size},
         low_priority_commands = Delayed0
        } = State0) ->
...
leader(EventType, Msg, State0) ->
    case handle_leader(Msg, State0) of
        ...
        {await_condition, State1, Effects1} ->
            {State, Actions} = ?HANDLE_EFFECTS(Effects1, EventType, State1),
            ?DEBUG_IF(is_command(Msg),
                      "~ts: postponing ~0P",
                      [log_id(State0), Msg, 10]),
            next_state(await_condition,
                       State,
    ...
    {State, Actions} = case ra_server:is_new(ServerState) of
                           ...
                           false ->
                               ?DEBUG("~ts: is not new, setting "
                                      "election timeout.",
                                      [log_id(State0)]),
                               maybe_set_election_timeout(TimeoutLen,
                                                          State1,
                                                          Actions0)
                       end,
    Monitors = ra_monitors:remove_all(machine, State#state.monitors),
    {keep_state, State#state{
                   low_priority_commands = ra_ets_queue:reset(Delayed),
                   monitors = Monitors
                  },
                 Actions};
...
follower(_,
         {command, Priority, {_CmdType, Data, noreply}},
         State) ->
    % forward to leader
...
follower(_, tick_timeout, #state{server_state = ServerState0} = State0) ->
    ServerState = ra_server:log_tick(ServerState0),
    {State, Actions} = ?HANDLE_EFFECTS([{aux, tick}],
                                       cast,
                                       State0#state{server_state = ServerState}),
    {keep_state, handle_tick_metrics(State),
                 set_tick_timer(State, Actions)};
...
receive_snapshot(enter, OldState, State0 = #state{conf = Conf}) ->
    ...
    {State, Actions} = handle_enter(?FUNCTION_NAME, OldState, State0),
    {keep_state, State,
                 [{state_timeout, ReceiveSnapshotTimeout, receive_snapshot_timeout} | Actions]};
...
await_condition(EventType, Msg, State0) ->
    case handle_await_condition(Msg, State0) of
        ...
        {await_condition, State1, Effects} ->
            ...
            %% liveness
            ?DEBUG_IF(is_command(Msg),
                      "~ts: await_condition postponing ~0P",
                      [log_id(State0), Msg, 10]),
...
terminate(Reason,
          StateName,
          #state{
            conf = #conf{name = Key, cluster_name = ClusterName},
...
handle_raft_state(RaftState,
                  Msg,
                  #state{
                    server_state = ServerState0,
                    election_timeout_set = Set
                   } = State) ->
...
handle_effect(RaftState, {aux, Cmd}, EventType, State0, Actions0) ->
    {_, ServerState, Effects} = ra_server:handle_aux(RaftState,
                                                     cast,
                                                     Cmd,
                                                     State0#state.server_state),
    {State, Actions} =
...
handle_effect(_, {cast, To, Msg}, _, State, Actions) ->
    ...
    {State, Actions};
handle_effect(RaftState,
              {reply, {Pid, _Tag} = From, Reply, Replier},
              _,
              State,
              Actions) ->
    case Replier of
...
handle_effect(leader,
              {send_snapshot, {_, ToNode} = To, {SnapState, Id, Term}},
              _,
              #state{
                server_state = SS0,
                monitors = Monitors,
                conf = #conf{
                         snapshot_chunk_size = ChunkSize,
...
handle_effect(_,
              ...
              _,  % EvtType
              #state{conf = #conf{aten_poll_interval = Poll}} = State,
              Actions) ->
...
handle_effect(RaftState,
    ...
    {ServerState, Effects} = ra_server:checkpoint(Index,
                                                  MacState,
                                                  ServerState0),
    State1 = State0#state{server_state = ServerState},
    handle_effects(RaftState, Effects, EvtType, State1, Actions0);
...
handle_effect(_,
    ...
    {State#state{monitors = ra_monitors:add(PidOrNode, machine, Monitors)},
     Actions0};
handle_effect(_,
              {monitor, _ProcOrNode, Component, PidOrNode},
              _,
              #state{monitors = Monitors} = State,
              Actions0) ->
    {State#state{monitors = ra_monitors:add(PidOrNode, Component, Monitors)},
     Actions0};
handle_effect(_,
              {demonitor, _ProcOrNode, PidOrNode},
              _,
...
handle_effect(_,
              {demonitor, _ProcOrNode, Component, PidOrNode},
              _,
              #state{monitors = Monitors0} = State,
              Actions) ->
...
handle_effect(follower, {record_leader_msg, _LeaderId}, _, State0, Actions) ->
    ...
    State = State0#state{
              leader_last_seen = erlang:system_time(millisecond),
              election_timeout_set = false
             },
...
follower_leader_change(Old,
                       #state{
                         pending_commands = Pending,
                         leader_monitor = OldMRef
                        } = New) ->
    ...
    case LeaderId of
        ...
        _ ->
            ?INFO("~ts: follower received leader command from ~w. "
                  "Rejecting to ~w ",
                  [log_id(State), Pid, LeaderId]),
            send_ra_event(Pid,
                          {not_leader, LeaderId, Corr},
                          id(State),
                          rejected,
                          State)
    end.
...
send_snapshots(Me,
    ...
    case SnapMacVer > TheirMacVer of
        true ->
            ok;
        false ->
            RPC = #install_snapshot_rpc{
                    term = Term,
                    leader_id = Id,
                    meta = Meta
                   },
            Result = read_chunks_and_send_rpc(RPC,
                                              To,
                                              ReadState,
                                              1,
                                              ChunkSize,
...
read_chunks_and_send_rpc(RPC0,
                         ...
                         InstallTimeout,
                         SnapState) ->
    {ok, Data, ContState} = ra_snapshot:read_chunk(ReadState0,
                                                   ChunkSize,
                                                   SnapState),
    ...
    %% rpcs
    RPC1 = RPC0#install_snapshot_rpc{
             chunk_state = {Num, ChunkFlag},
             data = Data
            },
    Res1 = gen_statem:call(To,
                           RPC1,
...
send_msg({send_msg, To, Msg, Options}, State) ->
    ...
    case parse_send_msg_options(Options) of
        {true, true} ->
            gen_cast(To,
                     wrap_ra_event(State, leader_id(State), machine, Msg),
                     State);
        {true, false} ->
            send(To,
                 wrap_ra_event(State, leader_id(State), machine, Msg),
...
can_execute_on_member(_RaftState,
                      Member,
...
handle_node_status_change(Node,
                          Status,
                          InfoList,
                          RaftState,
                          #state{
    ...
    {_, ServerState1, Effects} =
        lists:foldl(
          fun(Comp, {R, S0, E0}) ->
                  {R, S, E} = ra_server:handle_node_status(R,
                                                           Comp,
                                                           Node,
                                                           Status,
                                                           InfoList,
          ...
          {RaftState, ServerState0, []},
          Comps),
    ServerState = ra_server:update_disconnected_peers(Node,
                                                      Status,
                                                      ServerState1),
    {State, Actions} = handle_effects(RaftState,
                                      Effects,
                                      cast,
                                      State0#state{
                                        server_state = ServerState,
...
handle_process_down(Pid,
    ...
    {ServerState, Effects} =
        lists:foldl(
          fun(Comp, {S0, E0}) ->
                  {_, S, E} = ra_server:handle_down(RaftState,
                                                    Comp,
                                                    Pid,
                                                    Info,
                                                    S0),
...
maybe_record_cluster_change(#state{
    ...
    if
        (map_get(cluster_index_term, ServerStateA) =/=
